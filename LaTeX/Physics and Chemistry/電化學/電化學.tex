...
\item \tb{氧化劑（Oxidant or oxidizing agent, Ox）與還原劑（Reductant or reducing agent, Red）}：氧化劑指氧化還原反應中獲得電子者，即氧化數降低者；還原劑指氧化還原反應中失去電子者，即氧化數增加者。氧化劑、還原劑可對反應兩側定義，如：\ce{2I- + Br2 <=> 2Br- + I2}中，\ce{I-}、\ce{Br-}為還原劑，\ce{Br2}、\ce{I2}為氧化劑。氧化數小於最大常見氧化數、大於最小者常見氧化數者常兼具氧化性與還原性，如：\ce{NO2-}、\ce{H2O2}、\ce{SO2}、\ce{SO3^{2-}}、\ce{HSO3^{-}}。
...
\sssc{食品抗氧化劑（還原劑）}
\bit
\item 維生素C/抗壞血酸：可抗壞血病；可將食物中的\ce{NO2-}還原成NO，以防止\ce{NO2-}使血紅素中的\ce{Fe^{2+}}氧化，使失去攜氧能力。
...
\sssc{自身氧化還原反應}
\ce{X2(aq) + 2OH-(aq) -> X-(aq) + OX-(aq) + H2O(l)}、\ce{3X2(aq) + 6OH-(aq) -> 5X-(aq) + XO3-(aq) + 3H2O(l)}、\ce{P4(s) + OH-(aq) + H2O(l) -> PH3(g) + H2PO2-(aq)}、\ce{3MnO4^{2-}(aq) + 2H2O(l) -> 2MnO4-(aq) + MnO2(s) + 4OH-(aq)}、\ce{S2O3^{2-}(aq) + 2H+(aq) -> H2O(l) + SO2(g) + S(s)}、\ce{3HNO2(aq) -> HNO3(aq) + 2NO(g) + H2O(l)}、\ce{2KClO3(s) -> 2KCl(s) + 3O2(g)}、\ce{2H2O2(aq) -> 2H2O(l) + O2(g)}
...
\een
\ssc{無電（極）電鍍（Electroless plating）/非電解電鍍}
...
\item $E_{\text{reaction}}>0$表自發反應，$E_{\text{reaction}}<0$表非自發反應，$E_{\text{reaction}}=0$表反應達平衡。無外電動勢串聯時，反應的方向會趨向使得電動勢會逐漸下降，直到電動勢等於零時達平衡。
\end{itemize}
...
\item 正/陽極半反應\text{\ \{}\\
\texttt{if 正/陽極電極為活性電極\texttt{)}\ce{M(s) -> M^{n+}(aq) + ne-}\texttt{;}\\
...
\texttt{else\ }\ce{A^{n-}(aq) -> A(aq) + ne-}\texttt{;}\texttt{\}}
\end{itemize}
...
擬鍍鋅等還原電位小於\ce{H+}的金屬時，宜用鹼性電解液，因標準還原電位\ce{H+}>\ce{Zn^{2+}}>\ce{H2O}，鹼性條件可減少氫氣的干擾。
